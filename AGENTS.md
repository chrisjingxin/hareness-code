...
每个任务在标记完成前，必须使用 `open-code-review` Skill 对该任务的实际变更完成一次代码检视。审查时必须将任务需求、验收条件和已确认的方案设计作为 `--background`，并将范围限定为该任务的 commit 或 diff，避免混入他人无关改动。检视至少校验功能和验收项是否真正完成、架构设计与分层边界是否合理、是否存在安全问题，以及正确性、边界条件、可维护性、性能和整体代码质量问题。必须将审查命令、范围、结果和结论写入任务证据；所有高/中优先级问题必须修复后重新检视。确有无法解决的风险时，须记录原因和影响并获得用户确认，否则不得标记任务完成。
