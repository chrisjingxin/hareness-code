...
class SkillCatalogManager:
    ...
    def __init__(self, workspace: Path | str, *, home: Path | None = None) -> None:
        ...
        self._current: _SkillRegistry | None = None
        self._metadata_signature: tuple[object, ...] | None = None
        self._dirty = True

    @property
    def current(self) -> _SkillRegistry | None:
        """返回最近发布的 snapshot；不会隐式扫描文件系统。"""
        return self._current

    def refresh(self) -> _SkillRegistry:
        ...
        candidate = _SkillRegistry(self.workspace, home=self.home)
        ...
        if before != after:
            raise SkillError("Skill catalog changed during refresh")
        if current is None or self._dirty or current.snapshot_id != candidate.snapshot_id:
            current = candidate
