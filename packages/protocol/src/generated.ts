...
export const CLIENT_METHODS = ["initialize","run.start","run.cancel","config.show","config.path","shutdown"] as const
export const SERVER_METHODS = ["event","request"] as const
export const SERVER_CAPABILITIES = ["run.cancel","run.multithread","interactive.approval","interactive.question","config.read"] as const
export const EVENT_TYPES = ["run.started","content.delta","thinking.delta","tool.started","tool.delta","tool.completed","interaction.resolved","run.completed","run.cancelled","run.failed"] as const
