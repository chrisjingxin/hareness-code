...
export class JsonRpcPeer extends EventEmitter {
  ...
  startRun(message: string, threadId?: string, runId?: string, requestedSkill?: RequestedSkill, requestedModelProfile?: string): Promise<RunStartResult> {
    return this.call(Method.RUN_START, {
      message,
      thread_id: threadId,
      run_id: runId,
      requested_skill: requestedSkill,
      requested_model_profile: requestedModelProfile,
    }) as Promise<RunStartResult>
  }

  /** 兼容现有调用点的语义别名；wire 上已使用 run.start。 */
  query(message: string, threadId?: string, runId?: string, requestedSkill?: RequestedSkill, requestedModelProfile?: string): Promise<RunStartResult> {
    return this.startRun(message, threadId, runId, requestedSkill, requestedModelProfile)
  }
  ...
  /** 查询可用模型目录；传入 thread_id 时同时返回该 Thread 的不可变绑定。 */
