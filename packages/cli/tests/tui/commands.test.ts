...
test("解析核心 Slash Command 与别名", () => {
  ...
  expect(parseSlashCommand("/compact")).toEqual({ name: "compact", argument: undefined })
...
test("斜杠输入只展示已接入命令并按前缀过滤", () => {
  expect(findSlashCommands("/").map(item => item.name)).toEqual(["help", "quit", "clear", "force-clear", "compact", "status", "version", "resume", "skills"])
