...
> 评审门禁：已通过；正式任务为 [ZC-101](../tasks/ZC-101.md)～[ZC-107](../tasks/ZC-107.md)
...
- ZC-103 前 `SkillRegistry` 是进程级不可变快照，启动后 manifest 变化会要求重启；当前由 `SkillCatalogManager` 在顶层 Run 边界刷新：`packages/agent/harness_agent/skills.py`。
...
实现使用 typed 配置集中保存阈值和近期保留参数。ZC-105 用工具密集、文本密集和混合历史 fixture 校准出的默认值为：总体 `50% / 60% / 80% / 90%`，可回收工具结果达到 `8,192` token 或 `2` 个时提前尝试 micro，保留最近 `1` 个候选，idle 辅助阈值为 `900,000 ms`（15 分钟）。这些值保持现有总体水位兼容，不采用第三方固定数值；idle 默认开启，但只有 Run 生命周期标记的顶层首次模型调用可以使用它。
...
| 任务 | 目标 | 主要前置 |
| --- | --- | --- |
| [ZC-101](../tasks/ZC-101.md) | Append-only Thread 规范记录与完整 UI 历史 | ZC-088、ZC-089 |
| [ZC-102](../tasks/ZC-102.md) | RunContextSnapshot 与同 Thread 下一 Run 上下文刷新 | ZC-101 |
| [ZC-103](../tasks/ZC-103.md) | Skill Catalog 下一顶层 Run 热更新 | ZC-095、ZC-102 |
| [ZC-104](../tasks/ZC-104.md) | ContextProjector 与 CompressionCheckpoint | ZC-101 |
| [ZC-105](../tasks/ZC-105.md) | 分级压力策略与确定性微压缩 | ZC-104 |
| [ZC-106](../tasks/ZC-106.md) | 完整压缩与运行状态恢复闭环 | ZC-102、ZC-105 |
| [ZC-107](../tasks/ZC-107.md) | 唯一生产路径切换、迁移与端到端恢复 | ZC-103、ZC-106 |

当前设计文档仍作为任务拆分和边界依据；ZC-101～ZC-107 已完成，最终生产路径、测试证据和已接受遗留风险分别以各任务文档及 ADR 0003 为准。
...
- [x] 具体水位、工具阈值、空闲时长和近期保留数在 ZC-105 中通过测试确定。
