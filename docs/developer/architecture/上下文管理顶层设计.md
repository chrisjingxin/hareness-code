...
> 评审门禁：已通过；正式任务为 [HC-101](../task/archive/HC-101-Thread规范记录与Plug.md)～[HC-107](../task/HC-107-修复WebComposer、补.md)
...
- HC-103 前 `SkillRegistry` 是进程级不可变快照，启动后 manifest 变化会要求重启；当前由 `SkillCatalogManager` 在顶层 Run 边界刷新：`packages/agent/harness_agent/skills.py`。
...
| [HC-102](../task/HC-102-实现单实例WebHandoff.md) | RunContextSnapshot 与同 Thread 下一 Run 上下文刷新 | HC-101 |
| [HC-103](../task/archive/HC-103-让SkillCatalog在下.md) | Skill Catalog 下一顶层 Run 热更新 | HC-095、HC-102 |
| [HC-104](../task/HC-104-实现WebInteractiv.md) | ContextProjector 与 CompressionCheckpoint | HC-101 |
| [HC-105](../task/HC-105-建立WebBrowserE2E.md) | 分级压力策略与确定性微压缩 | HC-104 |
| [HC-106](../task/HC-106-按蓝色工作台设计整改WebUI.md) | 完整压缩与运行状态恢复闭环 | HC-102、HC-105 |
| [HC-107](../task/HC-107-修复WebComposer、补.md) | 唯一生产路径切换、迁移与端到端恢复 | HC-103、HC-106 |

当前设计文档仍作为任务拆分和边界依据；HC-101～HC-107 已完成，最终生产路径、测试证据和已接受遗留风险分别以各任务文档及 ADR 0003 为准。
