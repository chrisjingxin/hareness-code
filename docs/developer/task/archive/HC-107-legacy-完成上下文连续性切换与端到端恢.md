...
HC-101～HC-106 分别建立规范记录、Run 级上下文、Skill 热更新、模型投影、分级微压缩和完整压缩恢复。它们可以沿两条依赖链并行推进：

```text
HC-101 → HC-102 → HC-103
   └───→ HC-104 → HC-105 → HC-106
```
...
[HC-108](HC-108-legacy-Web代码高亮切换Shiki并.md)。本次提交不执行 `version:set`，也不作为正式发布节点；待 HC-108 关闭后，
...
- `tests/test_thread_persistence.py`、`tests/test_context_lifecycle.py`、
  ...
  多 waiter poison 二次检查；已有 HC-106 测试覆盖 micro/full/recovery、失败熔断和原子提交。
...
- 用户明确决定上述强制清理问题不再阻断 HC-107 提交。风险没有被描述为已修复，后续唯一任务为
  [HC-108](HC-108-legacy-Web代码高亮切换Shiki并.md)；在其完成前，本提交不作为正式发布节点。
