...
- 阶段 3 是阶段 7（WebUiGateway 的 `intent.outcome` 消息需要 accepted/rejected 载荷）与 HC-104 验收（Web 提交失败保留输入）的契约基础。
- 远程错误已由 HC-109 收敛到 AgentGatewayError，本任务只需在其上定义 RejectionCode 映射，无协议层改动。
...
- `InteractiveActivity`：删除 `label` 字段，kind 即契约；TUI 的 `timeline.tsx`/`theme.ts` 与 Web 的 presentation 各自维护 kind → 中文文案映射（`presentation-shared/timeline-presenter.ts` 在 HC-112 统一）。
...
- `runtime.ts` 的 `runtimeStatusSummary`/`approvalModeLabel` 等中文 label 保留在 runtime 展示函数（非领域状态），或在 HC-112 前仅移除"领域状态内"的文案；本任务边界：InteractiveState/Snapshot 零中文，展示函数不强制迁移。
...
- 不改 InteractiveIntent 集合与 AgentGateway 方法（HC-109 产物不动）。
