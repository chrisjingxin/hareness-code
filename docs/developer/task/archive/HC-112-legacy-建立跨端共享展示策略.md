...
方案 §10 表格定义七个共享模块：command-menu-policy、interaction-policy、timeline-presenter、semantic-tone、tool-output-policy、language-catalog、formatters。HC-108 已首建 `presentation-shared/language-catalog.ts`。
...
- 语言目录：HC-108 的 `presentation-shared/language-catalog.ts`（TUI syntax-parsers 与 Web Shiki loader 消费）。
...
| `timeline-presenter.ts` | Tool/Message/Interaction 展示语义（kind → 展示结构；中文文案映射集中于此，HC-110 的 TUI/Web 各自映射并入） | 具体组件和布局 |
...
| `language-catalog.ts` | canonical ID、alias、fallback（HC-108 已建，本任务补全 16 语言用例） | Grammar、parser、token |
...
- 高频 Timeline 与低频 Catalog 分片发布机制属于 HC-114（gateway revision），本任务只建立 Selector 函数与其单测。
...
- packages/protocol 不收上述模块（方案 §10：UI 通信契约放 CLI 内部，独立版本化——contracts 落位由 HC-114 执行）。
...
- 不改 InteractiveIntent/Outcome/Snapshot 领域形状；不做分片发布（HC-114）。
