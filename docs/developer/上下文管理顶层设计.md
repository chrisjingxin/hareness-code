...
> 评审门禁：已通过；正式任务为 [ZC-098](tasks/ZC-098.md)～[ZC-104](tasks/ZC-104.md)
...
- `PromptEpoch` 按 Thread 不可变，并保存完整 system prompt、AGENTS 快照和 Skill 索引：`packages/agent/harness_agent/prompting.py`。
- `RunContext` 已经是单次图调用的显式输入，适合承载新的 Run 上下文快照：`packages/agent/harness_agent/run_context.py`。
- `ContextWindowMiddleware` 当前已经按 50% 报告、60% 工具结果脱水、80% 摘要、90% 强制摘要分级处理；但达到摘要水位后不会先执行微压缩并重新计量，且最终直接使用 `RemoveMessage(REMOVE_ALL_MESSAGES)` 重写 LangGraph `messages`：`packages/agent/harness_agent/context_window.py`。
- `ThreadPersistence` 当前从 checkpoint 读取可恢复消息，并单独保存 PromptEpoch、Artifact、Summary 和压缩状态：`packages/agent/harness_agent/thread_persistence.py`。
- ZC-100 前 `SkillRegistry` 是进程级不可变快照，启动后 manifest 变化会要求重启；当前由 `SkillCatalogManager` 在顶层 Run 边界刷新：`packages/agent/harness_agent/skills.py`。
- Skill catalog 指纹已经进入 `AgentEngineProfile`，具备“新 Skill 快照 → 新 Profile”的身份基础：`packages/agent/harness_agent/agent_spec.py`。
- `RunCoordinator` 已集中拥有 Run 受理、执行、Interaction 和终态，是上下文准备和规范记录写入的正确编排边界：`packages/agent/harness_agent/run_coordinator.py`。
...
建议新增 `packages/agent/harness_agent/context_lifecycle.py`，作为一个深模块统一完成：
...
保留 `packages/agent/harness_agent/skills.py` 作为 Skill 深模块，但将“单个永久 Registry”调整为：
...
- `virtual_files.py` 不再闭包捕获 AgentEngine 构建时的唯一 Registry；它从当前 `RunContext` 取得本轮 Skill 快照。
- Skill 变化已经会改变 `AgentEngineProfile.skill_catalog_fingerprint`。新 Run 获取新 Profile，旧 Engine 进入定向 draining。
- 该行为依赖待实施的 [ZC-095](tasks/ZC-095.md)“共享资源与 AgentEngine 所有权”；实现时必须避免与 ZC-095 重复或产生两套失效机制。

### 4.3 ThreadTranscript：规范记录

在 `ThreadPersistence` 外部只暴露生命周期操作，SQLite 表和迁移继续留在 `thread_persistence.py` 内。
...
建议新增 `packages/agent/harness_agent/context_projection.py`，只负责：
...
建议新增 `packages/agent/harness_agent/context_pressure.py`。该模块是纯策略层，只接收当前有序投影、模型预算和调用阶段，返回压力快照与建议动作；它不修改消息、不调用模型、不写数据库。

实现使用 typed 配置集中保存阈值和近期保留参数。ZC-102 用工具密集、文本密集和混合历史 fixture 校准出的默认值为：总体 `50% / 60% / 80% / 90%`，可回收工具结果达到 `8,192` token 或 `2` 个时提前尝试 micro，保留最近 `1` 个候选，idle 辅助阈值为 `900,000 ms`（15 分钟）。这些值保持现有总体水位兼容，不采用第三方固定数值；idle 默认开启，但只有 Run 生命周期标记的顶层首次模型调用可以使用它。
...
| 目标位置 | 计划变化 | 原因 |
| --- | --- | --- |
| `packages/agent/harness_agent/prompting.py` | 将 Thread 永久 PromptEpoch 收窄为确定性 Prompt 组装能力；核心与可刷新块分离 | 支持 Qwen 风格 live System Instruction |
| `packages/agent/harness_agent/context_lifecycle.py` | 新增 Run 上下文准备与稳定排序深模块 | 避免 Server、Skill 和 Middleware 各自刷新 |
| `packages/agent/harness_agent/run_context.py` | `prompt_epoch` 改为 RunContextSnapshot，并携带本轮 Skill 快照/ID | 保证单 Run 一致性 |
| `packages/agent/harness_agent/skills.py` | 增加可刷新 Catalog Manager，继续产出 immutable Registry | 当前 Thread 后续 Run 使用新 Skill |
| `packages/agent/harness_agent/virtual_files.py` | 从 RunContext 解析本轮 Registry，不再捕获 Engine 构建期唯一 Registry | Skill 正文与索引、Profile 一致 |
| `packages/agent/harness_agent/agent_spec.py` | 使用 Run 准备阶段确定的 Skill snapshot；保持 Profile 指纹事实一致 | Skill 变化安全切换 Engine |
| `packages/agent/harness_agent/run_coordinator.py` | 在受理前取得统一 RunPreparation；追加完成的语义记录 | Run 生命周期是正确编排边界 |
| `packages/agent/harness_agent/thread_persistence.py` | 增加 Transcript、RunContextSnapshot、CompressionCheckpoint 的 typed 生命周期和 schema migration | SQLite 成为规范记录事实来源 |
| `packages/agent/harness_agent/context_projection.py` | 新增 latest checkpoint + tail 投影 | 分离完整历史与模型视图 |
| `packages/agent/harness_agent/context_pressure.py` | 新增纯压力策略：总体水位、工具压力、顶层空闲条件和动作决策 | 把“何时压缩”从消息改写与持久化中分离 |
| `packages/agent/harness_agent/context_window.py` | 收敛为投影上的微压缩、重新计量、完整压缩、溢出恢复和检查点草稿 | 复用现有 Artifact、摘要、熔断能力但不再覆盖规范记录 |
| `packages/agent/harness_agent/agent.py` | 调整 Middleware 顺序和投影同步入口，向压力策略传入模型调用阶段 | 在 LangChain/DeepAgents 中落地分级链路 |
| `packages/agent/harness_agent/server.py` | Host 持有 ContextLifecycle、SkillCatalogManager；删除永久 epoch 创建路径 | 维持 Project-scoped 资源所有权 |
| `packages/agent/tests/` | 增加资源刷新、Transcript/Projection、压缩、恢复、迁移和并发测试 | 覆盖生命周期而非私有表方法 |
| `docs/developer/架构总览.md` | 更新 Prompt、Thread、Context、Skill 生命周期 | 正式架构事实来源 |
...
| Skill 刷新与 Engine 构建竞态 | RunPreparation 一次取得 immutable Skill snapshot；ZC-095 定向 draining |
...
| 任务 | 目标 | 主要前置 |
| --- | --- | --- |
| [ZC-098](tasks/ZC-098.md) | Append-only Thread 规范记录与完整 UI 历史 | ZC-088、ZC-089 |
| [ZC-099](tasks/ZC-099.md) | RunContextSnapshot 与同 Thread 下一 Run 上下文刷新 | ZC-098 |
| [ZC-100](tasks/ZC-100.md) | Skill Catalog 下一顶层 Run 热更新 | ZC-095、ZC-099 |
| [ZC-101](tasks/ZC-101.md) | ContextProjector 与 CompressionCheckpoint | ZC-098 |
| [ZC-102](tasks/ZC-102.md) | 分级压力策略与确定性微压缩 | ZC-101 |
| [ZC-103](tasks/ZC-103.md) | 完整压缩与运行状态恢复闭环 | ZC-099、ZC-102 |
| [ZC-104](tasks/ZC-104.md) | 唯一生产路径切换、迁移与端到端恢复 | ZC-100、ZC-103 |

当前设计文档仍作为任务拆分和边界依据；ZC-100 已进入实现阶段，代码、直接相关测试和任务证据以目标仓库当前工作树为准，尚未完成固定窄评审。
...
- [x] 具体水位、工具阈值、空闲时长和近期保留数在 ZC-102 中通过测试确定。
- [x] Skill 热更新依赖 ZC-095 的定向失效能力。
