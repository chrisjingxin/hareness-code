# ZC-141：文件变更审批双栏与行内高亮 Diff 设计

> 原始需求：[ZC-141](../tasks/ZC-141.md)
...
prepared mutation，而不是“当前屏幕恰好显示出来的行”。这与 ZC-139 记录的产品决定一致。
...
- ZC-139 已过时：审批预览不要求完整显示，截断只需明确告知且继续允许批准或拒绝。
