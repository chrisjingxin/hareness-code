# ZC-102：单实例 Web Handoff 与 TUI 接管生命周期

原始需求：[ZC-102](../tasks/ZC-102.md)  
前置方案：[ZC-101：Host 控制租约与可撤销 Web Attachment](ZC-101.md)  
...
ZC-101 已让 Python Host 知道当前应由 TUI 还是 Web 接受输入，但 CLI 还没有管理“一次 Web 接管”的完整生命周期。现在的 `WebLauncher` 只启动静态页面并签发 attachment，TUI 继续工作，浏览器关闭、第二窗口、ready 超时和页面初始化失败也没有统一收敛路径。
...
它不代理 Agent JSON-RPC，不保存聊天 Timeline，不处理 approval/question，也不实现完整 Web UI。Browser 仍通过 ZC-101 的 loopback WebSocket 直连 Python Host。
...
- Browser 请求 `run.multithread`，没有请求或调用 `host.control.acquire/release`；ZC-101 完成后，其受控操作会被 Host 拒绝。
...
- ZC-101 已提供 `host.attachment.create/revoke`、`host.control.acquire/release/status`、稳定 `attachment_id` 和显式 attached capability ceiling；ZC-102 不修改这些语义。
...
- ZC-102 只维持当前简单 composer、历史和流式展示所需的最小行为；完整 Thread、模型、Skill、MCP、Slash Command 和 React presentation 留给 ZC-104。
...
- 锁定期间 TUI Controller 被卸载，因此不会保留 global Agent event listener 或 owner Interaction handler。ZC-101 保证 acquire 前 owner 没有 active Run/Interaction，卸载不会遗弃 owner 工作。
...
3. 接入 ZC-101 Host interface：owner 创建/revoke attachment并查询 status；Browser initialize/acquire/release；interactive CLI 请求 `host.control`。
...
5. 让简单 Web 页面支持 nullable Thread、第一条 Run 创建 Thread、`thread.changed` 和正常返回 TUI；不扩展 ZC-104 的完整功能。
...
OpenTUI 接管页属于用户可见变更，交付 PR 需附空首页打开、active 接管和 returning 三种终端截图。真实 Browser 跨进程矩阵、双视口和视觉基线仍由 ZC-105 完成。

## 非范围与交接

- 不实现 React DOM Web 工作台、完整 Thread 导航、模型/Skill/MCP 面板、完整 Slash Command 或视觉 parity；这些属于 ZC-104。
- 不提取 Interactive Core、`InteractiveController` 或迁移 TUI 业务语义；这些属于 ZC-103。ZC-102 只在 OpenTUI 根层处理宿主接管。
- 不新增 Protocol v3 operation/capability/event，也不改变 ZC-101 `ControlLease`、attachment 或 Run 语义。
- 不实现 active Run replay、页面刷新恢复、多 Browser 前台窗口、远程访问、daemon、登录或多用户。
- 不引入 Playwright；真实 Browser E2E、截图基线和完整安全矩阵属于 ZC-105。
