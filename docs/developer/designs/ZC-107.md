# ZC-107：Web Composer、离线代码高亮与界面精修

原始需求：[ZC-107：修复 Web Composer、补齐代码高亮并精修设计一致性](../tasks/ZC-107.md)

前置方案：[ZC-104：Web Interactive Adapter 与 React 工作台](ZC-104.md)、[ZC-106：蓝色 Web 工作台与显式深浅主题](ZC-106.md)

后续验收：[ZC-105：建立 Web Browser E2E 与完成验收闭环](../tasks/ZC-105.md)

## 通俗说明

ZC-106 已经把 Web 页面的大框架和颜色改到正确方向，但当前页面仍有三个会直接影响使用体验的问题：输入框在真实浏览器里不能工作，代码块没有像 TUI 一样按语法着色，Timeline 和控件细节没有还原设计稿的紧凑层级。
...
它不会重做 Web 架构。Thread、Run、Interaction、Model、Skill、MCP 和 Handoff 仍由 ZC-104 的共享 `InteractiveController` 与 Web Adapter 驱动；ZC-106 的显式 light/dark 主题、三栏骨架和移动端抽屉继续保留。
...
- 当前 ZC-106 未提交实现：`web/application/adapter.ts`、`web/presentation/*.tsx`、`styles.css` 及对应测试。
...
在 ZC-106 semantic token 上增加 scope 角色，不在 JSX 写 raw color：
...
- 保留 ZC-106 的 372px、四个 tab 与 Help 模式，不因截图中未打开就强制默认展开。
...
- 集成：真实 adapter + WebApp，原生 setter + `input`/composition/submit event 更新 DOM、snapshot 和 mock Controller；若 Happy DOM 仍无法承载，不能再次用 React 私有 props 替代该层证据，应把此用例放入 ZC-105 Browser smoke 并在 ZC-107 保留 Tabbit 手工证据。
...
每组记录：根 `scrollWidth <= clientWidth`、主要控件 bounding box、不重叠 hit area、输入/发送实际结果、代码 plain/highlighted text 一致。截图只作为 ZC-107 实施证据；ZC-105 再将这些场景自动化。
...
任务完成前按仓库规范运行 `open-code-review`，以 ZC-107 task、本文和实际 diff 为 background；所有高/中优先级问题修复后复检。
...
9. **交接 ZC-105**：把输入、IME、代码高亮 fallback、连续 Tool 和四组布局加入 Browser E2E/截图基线，避免 ZC-105 只验证旧场景。
...
- 不在本任务完成 ZC-105 的完整 Playwright/Host fixture、生命周期竞争和最终基线管理。
