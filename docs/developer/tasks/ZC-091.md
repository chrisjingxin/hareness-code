...
- [ ] AgentEngineProfile 与 AgentEngine builder 只消费同一个 ResolvedAgentSpec。
- [ ] 相同角色有效配置跨 Thread 复用 AgentEngine。
- [ ] 不同角色、模型、Policy、Prompt、能力视图或 sandbox 不会错误共用 AgentEngine。
- [ ] Team 组合和单次执行状态不进入 Profile key。
- [ ] 旧 AgentEngineProfile record 可读取但不会伪装成新角色身份。
