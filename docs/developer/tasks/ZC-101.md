...
references: docs/developer/上下文管理改造需求.md、docs/developer/上下文管理顶层设计.md、6a09aff
...
- 自动和手动压缩保留现有 50/60/80/90 水位、候选选择、摘要和 20% 节省规则；现有重写均先写 `mode=full` 检查点，没有提前实现 ZC-102/ZC-103。
...
- 固定 Luna Max 最终单项复审确认 falsey error 分支关闭，无新增 P0/P1，ZC-101 可进入完成检查。
