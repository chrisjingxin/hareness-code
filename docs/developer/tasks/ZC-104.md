...
[ZC-101](ZC-101.md) 建立 Append-only Transcript 后，完整本地历史已经有独立事实来源。但当前模型仍直接从 LangGraph checkpoint 的 `messages` 恢复，`ContextWindowMiddleware` 和手动 `/compact` 继续通过 `RemoveMessage(REMOVE_ALL_MESSAGES)` 把重写结果当成唯一历史。
