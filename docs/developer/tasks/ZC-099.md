...
owner: codex
branch: master
scope: 让 ZC-098 已启用且 trust 有效的 Plugin Skill 和 MCP 进入 Host 启动时的统一 SkillRegistry 与 McpConfigSnapshot，兼容 Agent Plugins 1.0 和 Claude 格式，并保持命名空间、最小进程环境和组件失败隔离。
...
test_evidence: Python: 531 passed, 1 skipped; TypeScript: 116 passed, 1 skipped; typecheck passed; protocol:check passed; docs/project check only blocked by a missing historical task reference in user-owned ZC-079
references: ZC-087、ZC-091、ZC-098
completed_at: 2026-07-30
---

## 背景

ZC-098 已能安全识别、安装、停用、启用 Agent Plugins 1.0 和 Claude Code Plugin，但组件报告
...
- 不实现运行中 Plugin 热重载；ZC-095 完成资源 ownership 前统一要求重启。
