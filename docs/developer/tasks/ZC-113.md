...
- [ ] `grep -rn "createInteractiveController" packages/cli/src/` 仅 index.ts（生产代码）；tui/app.tsx 无该引用。
- [ ] `tests/tui/web-handoff-root.test.tsx`：两次 handoff 往返后 adapter/controller identity 不变；返回后当前 Thread/Timeline 与 Web 侧一致（重同步生效）。
- [ ] web-active 期间 TUI 显示 WebTakeoverView（行为不变）；adapter 本地 draft/history 跨 handoff 保留。
- [ ] CLI shutdown：controller.close 在 runTui 返回后、agent.stop 前执行（顺序测试）。
