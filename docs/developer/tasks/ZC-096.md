---
id: ZC-096
title: 让动态 Delegation 通过 AgentEnginePool 执行
priority: P0
status: 待认领
owner: 未认领
branch: -
scope: 建立受控 delegate seam，让根 Agent 按 Agent ID 派发角色任务，并通过 resolver、执行绑定、AgentEnginePool、Policy、状态 namespace 和工作区调度完成子执行。
acceptance: Planner 与 Executor 等固定测试角色可以使用不同模型和工具 Policy 执行；相同角色配置复用 AgentEngine；每次子执行均可审计、取消、恢复并完整释放 lease。
...
target agent ID
  ...
  → AgentEnginePool.acquire(role profile)
...
校验目标与 delegation 边界
  ...
  → acquire AgentEngine + workspace lease
  → invoke child graph
...
- [ ] 相同 ResolvedAgentSpec 的子执行复用同一 AgentEngine 图。
- [ ] 每次子执行在 RunExecutionBinding 中有完整父子、模型、Policy 和终态记录。
- [ ] 父取消、子超时和异常都会释放 AgentEngine/workspace/resource lease。
- [ ] 修改一个角色只失效该角色 AgentEngine，不重建无关角色图。
