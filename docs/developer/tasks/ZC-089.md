...
`ActiveRun`、`_runs`、`_starting_threads`、`_registry_lock`、Connection pending request、Runtime lease 和 ThreadStore 分别保存部分状态。要判断一个 Run 是否已受理、可取消、正在 Interaction、已经终结或正在释放资源，需要跨多个字段推断。
