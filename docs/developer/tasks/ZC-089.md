...
owner: 未认领
branch: -
scope: 从 AgentHost 的 protocol implementation 中提取完整 Run lifecycle module，集中拥有受理、owner、幂等、Runtime lease、执行、Interaction、终态和资源释放。
acceptance: Protocol dispatcher 只负责校验、capability、wire 转换与 event fanout；Run 行为可通过一个稳定 interface 测试，不再依赖 Server private 方法。
user_docs: 不涉及
developer_docs: docs/developer/架构重构计划.md、docs/developer/架构总览.md、docs/developer/adr/0002-project-host-multi-connection.md
test_evidence: -
...
JSON-RPC v3 和 AgentHost 已经支持一个 Project 内的多个 Connection、单 Thread 单 active Run、Run owner、Interaction、RuntimePool 和事件 fanout。Host/Connection 领域边界已经建立，但 Run 的完整执行仍直接实现在 `server.py` 的 `AgentHost` 中。
...
`ActiveRun`、`_runs`、`_starting_threads`、`_registry_lock`、Connection pending request、Runtime lease 和 ThreadPersistence 分别保存部分状态。要判断一个 Run 是否已受理、可取消、正在 Interaction、已经终结或正在释放资源，需要跨多个字段推断。
...
- [ZC-086](ZC-086.md) 和 [ZC-088](ZC-088.md) 会提供类型化执行绑定与生命周期持久化 seam，此时提取 Run 不需要复制旧的 dict/schema 知识。
...
4. 接入 ZC-086 的 resolved binding 与 ZC-088 的 `accept_run()`，保持相同 Run ID 的幂等与冲突语义。
5. 将 Runtime lease、RunContext、stream loop、Interaction resume 和终态释放迁入 module。
...
- module 内集中执行 Thread busy/Run ID 幂等、Run owner、模型绑定、PromptEpoch、RuntimePool lease、DeepAgents stream、Interaction 和唯一终态。
...
- [ ] 每个 Run 恰好产生一个终态并完整释放 Runtime lease。
