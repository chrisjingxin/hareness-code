...
acceptance: /web 首次打开固定使用浅色；用户可在当前 Web 接管期间切换深浅主题；1440×900 与 390×844 下的品牌栏、顶栏、Thread 导航、Timeline、Tool、Interaction、Composer 和工具抽屉符合 ZC-106 设计且无横向溢出；所有现有 Web 工作流、权限和安全边界保持不变。
user_docs: docs/user/交互使用.md
developer_docs: docs/developer/designs/ZC-106.md
...
references: docs/developer/tasks/ZC-104.md、docs/developer/tasks/ZC-105.md、docs/developer/tasks/ZC-124.md
completed_at: -
---

## 背景

ZC-104 已将 `/web` 从最小 DOM 页面改为 React DOM 工作台，并通过共享 `InteractiveController` 覆盖 Thread、Run、Interaction、模型、Skill、MCP 和 Slash Command。页面功能已经具备，但当前视觉实现仍使用暖灰/琥珀 token、通用工具栏和另一套消息布局，与用户确认的蓝色 Web UI 设计稿以及 TUI 的深色开发工具气质差异明显。
...
- `styles.css` 同时保留 ZC-104 早期 class 和现有 presentation canonical class，视觉规则重复，后续维护容易出现覆盖顺序依赖。
- 用户提供的 390px 静态稿只隐藏桌面侧栏，实际渲染仍出现顶栏和 Composer 横向裁切；实现不能直接复制该响应式 CSS。

## 为什么现在要修改

ZC-105 将建立真实 Browser E2E 和 light/dark 截图基线。如果先对旧主题固化截图，再实施新设计，测试与基线需要重复建设。应先完成单一视觉整改，再由 ZC-105 对最终 UI 建立稳定验收证据。
...
- ZC-104 的 Adapter/Core 依赖方向、Handoff、capability、busy、Interaction、Markdown 和安全 invariant 全部保持不变。
...
完整接口、token、布局、状态和测试决策见 [ZC-106 方案设计](../designs/ZC-106.md)。
...
8. 更新 Web 用户说明；运行 CLI focused tests、build、typecheck 和项目级检查，再交给 ZC-105 建立真实浏览器双主题截图基线。
...
- 不在本任务建立 Playwright fixture 或提交最终视觉基线；该工作属于 ZC-105。
...
- [ ] active Run/Interaction 时的 Thread 切换和返回限制、capability 隐藏/禁用、Markdown 安全和 Handoff 规则与 ZC-104 保持一致。
...
- [ ] Web focused tests、`bun run build`、`bun run typecheck`、`bun run test` 与 `bun run project:check` 通过；随后由 ZC-105 生成最终四组浏览器截图。
...
- 2026-08-09（Codex）：任务的主题切换、三栏骨架和组件基础已经成为当前实现历史，但“蓝色工作台”作为未完成的最终视觉目标已被用户最新确认的 ZC-124“暖中性色画布 + 克制蓝色交互强调”取代。继续按本任务验收会与当前设计源冲突，因此标记为已过时；已有实现与测试证据保留，剩余响应式、视觉层级和可访问性分别由 ZC-125～ZC-130 接管。
