...
- Run 受理、完成、取消和重试已经由 [ZC-089](ZC-089.md) 的 `RunCoordinator` 集中拥有，适合在明确生命周期点追加语义记录。
...
- 并行 assistant 声明 A/B 而仅收到并提交 A 时，Transcript 有意保留“完整声明 + 已完成结果”的可检测不完整尾部；后续 ZC-101 必须拒绝把 unmatched tool-call group 直接投影回模型，ZC-098 不将其伪装成完整 LangChain 历史。
...
- v6 bootstrap 按 project-scoped checkpoint 导入当前仍可证明的消息，记录 `legacy_incomplete_history=1`，不生成虚假 run/execution 或 ZC-099 snapshot；迁移前备份只作恢复材料，事务失败优先 rollback。
