---
id: ZC-103
title: 提取共享交互会话内核并迁移 TUI
priority: P0
status: 待认领
owner: 未认领
branch: -
scope: 从现有 TuiController 提取表现层无关的 InteractiveSessionController，并让 TUI 完整迁移到该 interface，统一 Run、Timeline、Interaction、Thread、catalog 和 Slash Command 语义。
acceptance: TUI 的聊天与命令行为全部通过共享 Controller interface 驱动；应用语义不再依赖 OpenTUI 或 DOM；同一 intent 在不同 adapter 下产生相同 RPC 与领域 snapshot；旧重复实现和透传 wrapper 被删除。
user_docs: 不涉及
developer_docs: docs/developer/architecture/架构总览.md、docs/developer/architecture/斜杠命令体系.md
test_evidence: -
references: docs/developer/architecture/斜杠命令体系.md
completed_at: -
---

## 背景

当前 `TuiController` 同时拥有 Prompt 编辑、OpenTUI picker/dialog、Slash Command、Run、Timeline reducer、Interaction、Thread 恢复、模型选择、Skill 和 MCP 工作流。Web 若直接复用这个类会被终端细节污染；若另写一个 controller，又会产生两套业务语义。

本任务先建立表现层无关的交互会话深模块，并把现有 TUI 迁移成第一个 adapter。ZC-104 只能复用这个已被 TUI 验证过的 interface，不能先为 Web 新增平行实现。

## 当前存在的问题

- Run/Event/Interaction 与 TUI draft、光标、overlay、滚动状态位于同一个大类。
- CommandRegistry 已统一声明命令，但副作用解释仍只存在于 TUI Controller。
- Thread/模型/Skill/MCP catalog 的加载、错误和选择状态没有可供第二个 adapter 使用的领域 snapshot。
- 现有测试主要通过 TUI 专用 snapshot 验证；若再新增 Web controller，语义漂移无法被契约测试发现。

## 为什么现在要修改

共享应用层是 Web parity 的前置条件。只有先让现有 TUI 通过新 seam 工作，才能证明该 module 真正隐藏了业务复杂度，而不是为 Web 创建一个无人复用的浅层 wrapper。

## 目标设计

`InteractiveSessionController` 提供小 interface：

```text
dispatch(InteractiveIntent) → Promise<InteractiveResult | void>
getSnapshot()              → InteractiveSnapshot
subscribe(listener)        → unsubscribe
close()                    → Promise<void>
```

它内部拥有：

- Timeline reducer、active Run、取消和唯一终态；
- approval/question 的动态 schema、超时与 response；
- nullable current Thread、恢复和空首页；
- 模型、Skill、MCP catalog 与选择/写入；
- CommandRegistry、availability 和 command dispatcher；
- semantic notice/status，不包含终端尺寸、DOM、颜色或组件状态。

TUI adapter 只拥有 draft/光标、Prompt history、picker/dialog 展示、快捷键、滚动和 OpenTUI refs。adapter 将用户动作转换为 `InteractiveIntent`，不直接调用 AgentClient 业务方法。

## 实施步骤

1. 列出 `TuiController` 当前字段和方法，按“共享语义 / TUI presentation”分类，先固定 `InteractiveSnapshot` 与 `InteractiveIntent`。
2. 将 reducer、Run drain、Interaction handler、Thread/catalog/model/Skill/MCP 工作流移入 `InteractiveSessionController`；依赖通过现有 AgentClient interface 注入。
3. 让 Slash Command dispatcher 返回表现层无关的 semantic intent/result；help、status、参数校验、capability 和 busy 条件只实现一次。
4. 改造 TUI Controller 为 adapter，所有 Agent RPC 和领域状态均通过共享 Controller；保留 OpenTUI 专属 overlay 与输入体验。
5. 删除旧的 Run/Event/Interaction/catalog 重复字段和方法，不保留 alias、fallback 或双写。
6. 将旧测试迁移到共享 interface；新增同一 intent 序列的 snapshot/RPC 契约测试，再保留少量 TUI adapter 测试验证键盘和 overlay 映射。
7. 更新架构总览和 Slash Command 文档，说明共享 module 与两个 adapter 的 seam。

## 范围

- CLI application 层的共享会话 interface、implementation、intent 和 snapshot。
- TUI adapter 的完整迁移。
- Timeline、Run、Interaction、Thread、模型、Skill、MCP、compact、status 和 Slash Command 语义。
- interface 级测试和 TUI adapter 回归。

## 非范围

- 不实现 React DOM 或 Web 页面布局。
- 不实现 WebSessionCoordinator、attachment 或 Host 控制租约。
- 不改变 Agent 业务规则或新增 TUI 当前没有的产品功能。
- 不保留新旧 Controller 双运行或状态同步层。

## 验收清单

- [ ] TUI 生产路径创建并使用 `InteractiveSessionController`，旧 TUI 业务实现已删除。
- [ ] TUI adapter 不直接调用 Run、Thread、模型、Skill、MCP 和配置写 RPC。
- [ ] CommandRegistry 的解析、可用性、参数校验和执行只存在一条 canonical 路径。
- [ ] approval/question、取消、Thread 切换、空首页和模型选择均通过共享 snapshot 可观察。
- [ ] `thread.changed` 能明确表达 `string | null`，清空 Thread 不会保留旧身份。
- [ ] interface 测试覆盖正常、失败、取消、超时、乱序/重复 Event 和 catalog 部分失败。
- [ ] 删除 module 后复杂度会重新散落到 adapter，证明该 seam 具有实际 depth。

## 前置

- 无；可与 ZC-101 并行，但 ZC-104 必须等待本任务完成。
