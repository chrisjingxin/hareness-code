...
[ZC-101](ZC-101.md) 把这些结果迁移为模型投影检查点，[ZC-102](ZC-102.md) 建立“先微压缩、重新计量、仍不足再升级”的分级策略。本任务完成剩余的完整压缩、原子提交和压缩后连续性，对应 `REQ-CTX-006` 与 `REQ-CTX-007`。
...
```text
当前投影
→ 自动路径先接受 ZC-102 的 micro projection；手动路径显式 force full
→ 选取可摘要前缀和必须保留的最近原子组
→ 调用无工具摘要模型
→ 校验内容、长度、来源、结构和节省量
...
6. 自动路径只在 ZC-102 重新计量后仍超过 full watermark 时调用；同一轮 micro + full 只提交最终 full 检查点。
...
- [ ] 自动路径只在微压缩后仍超过 full watermark 时调用摘要模型。
- [ ] 微压缩足够时不生成 full checkpoint，也不消耗摘要模型调用。
- [ ] 手动 `/compact` 明确尝试完整压缩；短历史、无有效前缀或节省不足时返回 skipped 而非假成功。
- [ ] overflow 复用同一服务且最多重试一次，不形成递归压缩循环。
- [ ] 空、超长、截断、膨胀、来源错误或低节省摘要不会覆盖上一个有效检查点。
- [ ] Artifact、Summary、full checkpoint 和 ContextState 原子提交。
- [ ] 压缩后 Todo、执行/审批模式和 Artifact 引用从结构化状态恢复。
- [ ] AGENTS、Skill 和工具能力不进入摘要，模型继续使用当前 RunContextSnapshot。
- [ ] 连续多次 full 压缩只使用 latest valid checkpoint + tail，所有旧检查点仍可审计。
- [ ] 三次自动失败后熔断，成功后重置；手动路径不受自动熔断阻止但仍受结果校验。
...
本任务改变 `/compact`、自动压缩和 overflow 的用户可见行为。实现完成时必须更新用户文档、记录最终阈值依赖与迁移证据，并通过项目版本流程决定版本。
