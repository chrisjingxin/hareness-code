...
- [ZC-104](ZC-104-legacy.md) 已使压缩只作用于模型投影，并支持 `mode=micro` 检查点。
...
7. 微压缩单独足够时通过 ZC-104 原子提交 `mode=micro` 检查点；仍需 full 时不提交中间检查点，只把内存投影交给后续完整压缩。
...
且本改动是尚未发布的上下文改造链中间步骤，后续仍由 ZC-106 完成 full 闭环、
ZC-107 完成最终用户行为切换。正式 SemVer 与 Changelog 统一在 ZC-107 的版本流程中决定，
避免把未形成完整闭环的内部增量单独发布。

## ZC-105 实施记录（2026-08-01）
...
- 固定 Luna Max 增量评审确认 3 个 P1 已关闭，无新增 P0/P1，ZC-105 可进入完成检查。
...
本任务不包含 ZC-106/ZC-107 的完整恢复链、手动压缩新语义或长期记忆。
