...
references: docs/developer/tasks/ZC-107.md
...
ZC-107（进行中）的设计文档曾明确"仓库已锁定 `web-tree-sitter@0.25.10`，不需要引入 Shiki"；最终架构方案（2026-08-04）推翻了该决策。本任务先落地 Shiki 替换，ZC-107 后续只保留 Composer 修复与设计精修部分，其代码高亮范围以本任务结果为准。
...
- ZC-107 正在推进 tree-sitter 高亮，若不在其完成前切换，会产生返工；本任务应先于 ZC-107 高亮验收落地。
...
8. 更新 `docs/user/交互使用.md`（Web 代码高亮能力与降级行为）；更新 `tests/web/architecture.test.ts` 断言 web/syntax 不再 import `tui/platform`；按仓库规范更新 ZC-107 的 scope 说明（高亮部分由本任务接管）。
...
- 不重构 Markdown 解析、Timeline、Composer 与 ZC-107 其余范围。
