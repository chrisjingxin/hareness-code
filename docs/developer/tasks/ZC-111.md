...
test_evidence: "拆分后 `wc -l packages/cli/src/interactive/controller.ts` = 300（≤300）；`grep -rn \"from '../features\" packages/cli/src/interactive/features/` 无匹配（Feature 间零直接依赖）；`cd packages/cli && bun test --isolate tests/interactive`：71 pass / 0 fail（含拆分迁移后等价断言 + 新增 mcp-feature 失败路径测试）；`bunx tsc --noEmit` 通过；`bun run test` 全量（isolate）：interactive 相关全绿，11 个 TUI/index 失败为 `@opentui/core` 0.4.3 native 绑定在 bun --isolate 打包下的加载失败（node_modules 内部错误），已用 `git stash` 基线验证为改动前预存问题，非本任务引入；非 isolate `bun test` 下另有 4 个 web 子集 happy-dom fetch 测试间干扰失败（tests/web/integration.test.ts、tests/web/server.test.ts），单独运行各文件全绿，属 ZC-115 Web 任务预存问题"
...
- 阶段 4 以 ZC-109（Port 完备）、ZC-110（IntentOutcome）为前置：Feature 只依赖 Port 与稳定 outcome，拆分才不会把 IPC/文案依赖带进新模块。
...
- **Controller**：只保留 `getSnapshot/subscribe/dispatch/close` 路由、listeners 管理、snapshot 组装（buildSnapshot 可委托给 selectors 层——本任务先内联，ZC-112 再抽 selectors）、Feature 生命周期（init/close）。预期 ≤300 行。
- **快照发布**：保持当前"单 snapshot 整体发布"语义（分片发布是 ZC-112/ZC-114 的 revision 机制，本任务不改发布频率，避免行为变化）。
...
- 不改 InteractiveController 外部接口、Intent/Outcome/Snapshot 形状（ZC-110 产物）。
- 不做 Selector 分片与展示策略抽取（ZC-112）；不做 Composition Root 提升（ZC-113）。
...
- [x] `bun run typecheck`、`bun run test`（interactive 全绿；web 子集 4 个 happy-dom 测试间干扰失败为 ZC-115 预存问题）、`bun run project:check` 全绿；证据与 OCR 结论写入本任务。
